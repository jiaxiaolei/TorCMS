# -*- coding:utf-8 -*-

from torlite.core import tools

from torlite.model.core_tab import *



class MRelation(object):
    def __init__(self):
        self.tab_relation =CabRelation
        self.tab_post = CabPost
        try:
            CabRelation.create_table()
        except:
            pass

    def add_relation(self, app_f, app_t):
        cur = self.tab_relation.select().where((self.tab_relation.app_f == app_f) & (self.tab_relation.app_t == app_t))
        if cur.count() > 1:
            for x in cur:
                self.delete_by_uid(x.uid)

        if cur.count() == 0:
            uid = tools.get_uuid()
            print('=' * 80)
            entry = self.tab_relation.create(
                uid=uid,
                app_f=app_f,
                app_t=app_t,
                count=1,
            )
            return entry.uid
        elif cur.count() == 1:
            self.update_relation(app_f, app_t)
        else:
            return False

    def delete_by_uid(self, uid_base, uid_rel):
        entry = self.tab_relation.delete().where((self.tab_relation.app_f == uid_base)  & (self.tab_relation.app_t ==  uid_rel))
        entry.execute()

    def update_relation(self, app_f, app_t):
        try:
            uu = self.tab_relation.get((self.tab_relation.app_f == app_f) & (self.tab_relation.app_t == app_t))
        except:
            return False
        entry = self.tab_relation.update(
            count=uu.count + 1
        ).where((self.tab_relation.app_f == app_f) & (self.tab_relation.app_t == app_t))
        entry.execute()

    def get_app_relations(self, app_id):
        return self.tab_relation.select().join(self.tab_post ).where(self.tab_relation.app_f == app_id).order_by(
            self.tab_relation.count.desc()).limit(20)
